...
void backend(GameInfoExtended_t *game_info, UserAction_t *user_action,
  ...
  Automaton_t *automaton = getAutomaton(); // Retrieves the current state of the
                                           // game's state machine (automaton).

  if (*automaton == Spawn) {
    spawn_figure_to_game(game_info); // Spawn a new figure onto the game field.
    *collision = 0;                  // Reset collision flag.
    *automaton = Moving;             // Transition to the Moving state
  } else if (game_info->pause) {     // where the figure can move down.
    clear_cache_terminal(); // Clear any cached output or temporary data on the
                            // terminal.
    game_info->pause = 0;   // Unpause the game.
    *user_action = Start;   // Set the user action to start/resume the game.
  } else if (*automaton == Moving) {
    if (skip_tick(game_info, tick_count)) {
      *automaton = Shifting; // If we need to skip a tick (e.g., for a delay),
    } else {                 // transition to Shifting state.
      if (*user_action == Pause) {
        game_info->pause = 1; // If the user action is to pause,
      } else {                // set the game to pause.
        game_move_procces(game_info, user_action,
                          collision); // Process the game
      }                               // move based on user action.
      if (*collision)
        *automaton = Collided; // If there's a collision,
                               // transition to the Collided state.
      else if (*user_action == Terminate)
        *automaton = GameOver; // If user action is to terminate,
    }                          // transition to the GameOver state.
  } else if (*automaton == Collided) {
    score_update(game_info); // Update the score after a collision has occurred.
    copy_next_to_field(game_info, GAME_FIELD_Y_END, GAME_FIELD_X_END);
    *automaton = Attaching;             // Transition to Attaching state
  } else if (*automaton == Attaching) { // to attach the figure to the stack.
    if (can_spawn_figure(game_info))
      *automaton = Spawn;    // If a new figure can spawn,
    else                     // transition back to Spawn state.
      *automaton = GameOver; // If a new figure can't spawn, the game is over.
  } else if (*automaton == Shifting) {
    *automaton = Moving; // After shifting, go back to
  } else if (*automaton ==
             Start_automaton) { // moving state to continue game flow.
    *automaton = Spawn;         // If the automaton is at the start,
  }                             // begin the game by spawning a figure.
  copy_game_info_extended_to_api(game_info); // Synchronize the extended game
} // information with the game's API.
...
void game_move_procces(GameInfoExtended_t *game_info, UserAction_t *user_action,
  ...
  *user_action =
  ...
  if ((*collision = is_collision(game_info)) == 0) {
    // Move the figure based on the user action
    move_figure(user_action, game_info);
  }
  napms(10); // Introduce a short delay to regulate game phace
}
 ...
 *
 * This function reads the current best score from the "best_score_tetris.txt"
 * file, compares it with the current game score, and updates the best score if
 * the current game score is higher. If the file does not exist, it creates a
 * new file. If the file cannot be opened, it prints an error message.
 *
 * @param game_info Pointer to the GameInfoExtended_t structure containing
 * extended game information, including the current game score.
 */

void updateBestScore(GameInfoExtended_t *game_info) {
  // Open the file in write mode to overwrite the current best score
  FILE *f_best_score = fopen("brickgame/tetris/best_score_tetris.txt", "w");
  if (f_best_score != NULL) {
    ...
    fclose(f_best_score); // Close the file after writing
  } else {
...
/**
 * @brief Updates the game score, best score, and level based on the game state.
 *
 * This function calculates the number of full rows in the game field, updates
 * the game score based on the filled rows, checks for a new high score, and
 * processes level up conditions. It encapsulates the logic for updating the
 * game's score, best score, and level based on the current game state.
 *
 * @param game_info Pointer to the GameInfoExtended_t structure containing
 * extended game information, including the current game score, high score,
 * level, and game field state.
 */
void score_update(GameInfoExtended_t *game_info) {
  ...
  // Update the score based on filled rows
  add_score(game_info, count_fill_rows);
  if (game_info->high_score < game_info->score)
    updateBestScore(
        game_info); // Update best score from file best_score_tetris.txt
