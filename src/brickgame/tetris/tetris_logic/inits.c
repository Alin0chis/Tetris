 ...
 *
 * This function reads the best score from the "best_score_tetris.txt" file and
 * updates the high score attribute of the game information structure. If the
 * file does not exist or an error occurs while reading, the high score is set
 * to 0.
 *
...
void getBestScore(GameInfoExtended_t *game_info) {
  FILE *f_best_score = fopen("brickgame/tetris/best_score_tetris.txt", "r");
