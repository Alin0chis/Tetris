...
/**
 * @brief Clears the current figure from the game field and resets its position.
 *
 * This function clears the current figure from the game field by setting all
 * its bricks to empty. It also resets the x and y coordinates of the current
 * figure to 0, effectively resetting its position to the top-left corner of the
 * game field.
 *
 * @param game_info A pointer to the GameInfoExtended_t struct containing the
 * game state.
...
void clear_current_figure(GameInfoExtended_t *game_info) {
  ...
  // Reset the x-coordinate of the current figure.
  game_info->figure_current->x_figure = 0;
  // Reset the y-coordinate of the current figure.
...
void get_figure_next(GameInfoExtended_t *game_info) {
  clear_next_figure(game_info); // Clears the next figure.
...
void spawn_figure_to_game(GameInfoExtended_t *game_info) {
  // Updates the game field with the current figure.
...
/**
 * @brief Checks if the current figure can move to the right in the game field.
 *
 * This function checks if the current figure can move to the right in the game
 * field by iterating through the figure and checking for empty spaces or the
 * absence of collision with existing bricks in the game field.
 *
 * @param game_info A pointer to the GameInfoExtended_t struct containing the
 * game state.
 *
 * @return 1 if the figure can move to the right, 0
 *  otherwise.
 */
int can_move_right(GameInfoExtended_t *game_info) {
  // Initialize the flag for rightward movement feasibility.
  int can_move_left = 1;

  int y_figure = game_info->figure_current->y_figure;
  int x_figure = game_info->figure_current->x_figure;

  // Iterate through the cells of the current figure to check for empty spaces
  // or absence of collision with existing bricks in the game field.
  for (int i = 3; i >= 0; i--) {
    for (int j = 3; j >= 0; j--) {
      if (game_info->figure_current->figure[i][j] == BRICK) {
        if (game_info->field[i + y_figure][j + x_figure + 1] == BRICK ||
            (j + x_figure + 1) == GAME_FIELD_X_END) {
          can_move_left = 0;
        }
      }
    }
  ...
  return can_move_left; // Return the flag indicating whether the figure can
                        // move to the right.
}

/**
 * @brief Checks if the current figure can move to the left in the game field.
 *
 * This function checks if the current figure can move to the left in the game
 * field by iterating through the figure and checking for empty spaces or the
 * absence of collision with existing bricks in the game field.
 *
 * @param game_info A pointer to the GameInfoExtended_t struct containing the
 * game state.
 *
 * @return 1 if the figure can move to the left, 0 otherwise.
 */
int can_move_left(GameInfoExtended_t *game_info) {
  int can_move_left =
      1; // Initialize the flag for leftward movement feasibility.
  ...
  for (int i = 3; i >= 0; i--) {
    for (int j = 3; j >= 0; j--) {
      if (game_info->figure_current->figure[i][j] == BRICK) {
        if (game_info->field[i + y_figure][j + x_figure - 1] == BRICK ||
            (j + x_figure) == 0) {
          // Set the flag to 0 if leftward movement is not feasible.
          can_move_left = 0;
        }
      }
    }
...
/**
 * @brief Checks if the current figure can move down in the game field.
 *
 * This function checks if the current figure can move down in the game field by
 * iterating through the figure and checking for empty spaces or the absence of
 * collision with existing bricks in the game field.
 *
 * @param game_info A pointer to the GameInfoExtended_t struct containing the
 * game state.
 *
...
int can_move_down(GameInfoExtended_t *game_info) {
  int can_move = 1; // Initialize the flag for downward movement feasibility.
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (game_info->figure_current->figure[i][j] == BRICK) {
        if (game_info->field[i + y_figure + 1][j + x_figure] != EMPTY) {
          can_move = 0; // Set the flag to 0 if
        }               // downward movement is not feasible.
      }
    }
  }

  return can_move; // Return the flag indicating whether the figure can move
                   // down.
}
/**
...
/**
 * @brief Moves the current figure to the right in the game field.
 *
 * This function moves the current figure to the right in the game field by
 * clearing the current figure from its current position and updating the
 * x-coordinate of the figure's position.
 *
 * @param game_info A pointer to the GameInfoExtended_t struct containing the
 * game state.
 */
...
void rotate_figure(GameInfoExtended_t *game_info, int **temp_figure) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      // Update the current figure with the rotated figure.
      game_info->figure_current->figure[i][j] = temp_figure[i][j];
    }
  }
}
...
void copy_figure_next_to_current(GameInfoExtended_t *game_info) {
  // Clear the current figure from the game field.
  ...
  game_info->figure_current->y_figure = 0;
  // Reset the x-coordinate of the current figure.
  game_info->figure_current->x_figure = FIGURE_CENTR_X;
}
/**
...
int can_spawn_figure(GameInfoExtended_t *game_info) {
  ...
  int y_figure = game_info->figure_next->y_figure;
  int x_figure = game_info->figure_next->x_figure;
