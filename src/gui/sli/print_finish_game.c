...
void print_close_game(GameInfo_t game_info) {
  WINDOW *fail_win = newwin(GAME_WALL_Y_END, GAME_WALL_X_END * 2 + 5, 0,
                            0); // Create a new window for displaying
                                // the close game message
  box(fail_win, 0, 0);          // Draw a box around the window
  refresh();                    // Refresh the standard screen

  char text[30];
  // Format the player's score into a text message
  sprintf(text, "Your score %d", game_info.score);
  // Print the player's score to the center of the screen
  print_text_to_centr(text, 10);
  // Print instructions to close the game to the center of the screen
